...
func diffEntries(prevEntries, newEntries []registry.Entry) []registry.Operation {
	actions := make([]registry.Operation, 0)
	...
	for path, prevEntry := range prevMap {
		if newEntry, ok := newMap[path]; !ok {
			actions = append(actions, registry.Operation{Kind: registry.Delete, Entry: *prevEntry})
		} else if newEntry.Data != prevEntry.Data { // todO; || newEntry.Meta != prevEntry.Meta
			actions = append(actions, registry.Operation{Kind: registry.Update, Entry: *newEntry})
		}
	}
	...
	for path, newEntry := range newMap {
		if _, ok := prevMap[path]; !ok {
			actions = append(actions, registry.Operation{Kind: registry.Create, Entry: *newEntry})
		}
	}
