...
func Test_history_addVersion(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			h := newHistory()
			//pre-populate history and versions
			...
			if err := h.addVersion(tt.args.v, tt.args.actions); (err != nil) != tt.wantErr {
				t.Errorf("history.addVersion() error = %v, wantErr %v", err, tt.wantErr)
			}
			if len(h.entries) != len(tt.wantHistory) {
				t.Fatalf("history.addVersion() entries length = %v, want %v", len(h.entries), len(tt.wantHistory))
			}
			for i, entry := range h.entries {
				wantEntry := tt.wantHistory[i]
				if entry.version != wantEntry.version {
					t.Errorf("history.addVersion() entry version = %v, want %v", entry.version, wantEntry.version)
				}
				if len(entry.actions) != len(wantEntry.actions) {
					t.Fatalf("history.addVersion() actions length = %v, want %v", len(entry.actions), len(wantEntry.actions))
				}
				for j, action := range entry.actions {
					wantAction := wantEntry.actions[j]
					if action.Kind != wantAction.Kind {
						t.Errorf("history.addVersion() action Kind = %v, want %v", action.Kind, wantAction.Kind)
					}
					if action.Entry.Path != wantAction.Entry.Path {
						t.Errorf("history.addVersion() action Entry.Path = %v, want %v", action.Entry.Path, wantAction.Entry.Path)
					}
					if action.Entry.Data.Format() != wantAction.Entry.Data.Format() {
						t.Errorf("history.addVersion() action Entry.Data format = %v, want %v", action.Entry.Data.Format(), wantAction.Entry.Data.Format())
					}
					if action.Entry.Data.Data() != wantAction.Entry.Data.Data() {
						t.Errorf("history.addVersion() action Entry.Data data = %v, want %v", action.Entry.Data.Data(), wantAction.Entry.Data.Data())
					}
				}
			}

			if len(h.versions) != len(tt.wantVersions) {
				t.Fatalf("history.addVersion() versions length = %v, want %v", len(h.versions), len(tt.wantVersions))
			}

			for version, entries := range h.versions {
				wantEntries := tt.wantVersions[version]
				if len(entries) != len(wantEntries) {
					t.Fatalf("history.addVersion() entries length for version %v = %v, want %v", version, len(entries), len(wantEntries))
				}
				for i, entry := range entries {
					wantEntry := wantEntries[i]
					if entry.Path != wantEntry.Path {
						t.Errorf("history.addVersion() entry Path for version %v = %v, want %v", version, entry.Path, wantEntry.Path)
					}
					if entry.Data.Format() != wantEntry.Data.Format() {
						t.Errorf("history.addVersion() entry Data format for version %v = %v, want %v", version, entry.Data.Format(), wantEntry.Data.Format())
					}
					if entry.Data.Data() != wantEntry.Data.Data() {
						t.Errorf("history.addVersion() entry Data data for version %v = %v, want %v", version, entry.Data.Data(), wantEntry.Data.Data())
					}
				}
			}
		})
