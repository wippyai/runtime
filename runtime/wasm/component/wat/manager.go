// Package wat provides management for WAT (WebAssembly Text) functions.
package wat
...
import (
	...
	"github.com/wippyai/runtime/runtime/wasm/component"
...
// configEntry holds config for a WAT function.
...
// Manager handles WAT function compilation, pooling and execution.
type Manager struct {
	ctx        context.Context
	log        *zap.Logger
	bus        event.Bus
...
// NewManager creates a new WAT function manager.
func NewManager(
	log *zap.Logger,
	bus event.Bus,
...
func (m *Manager) Start(ctx context.Context) error {
	...
	m.log.Info("wat function manager started")
	return nil
}
...
func (m *Manager) Stop() {
	...
	m.log.Info("wat function manager stopped")
}

// Add is called when a new entry is created.
func (m *Manager) Add(ctx context.Context, entry registry.Entry) error {
	if entry.Kind != wasmapi.FunctionWAT {
		return runtimewasm.NewInvalidEntryKindError(entry.Kind, wasmapi.FunctionWAT)
	}

	cfg, err := component.UnpackConfig[wasmapi.WATFunctionConfig](ctx, entry)
	if err != nil {
		return runtimewasm.NewUnpackConfigError("wat function", err)
	}
	...
	m.log.Debug("wat function added",
...
func (m *Manager) Update(ctx context.Context, entry registry.Entry) error {
	...
	m.log.Debug("wat function updated", zap.String("id", entry.ID.String()))
	return nil
}
...
func (m *Manager) Delete(_ context.Context, entry registry.Entry) error {
	...
	m.log.Debug("wat function deleted", zap.String("id", entry.ID.String()))
	return nil
}
...
func (m *Manager) createPool(ctx context.Context, id registry.ID, cfg *configEntry) error {
	asyncImports := m.buildAsyncImports()

	module, err := engine.CompileWATWithOptions(ctx, m.runtime, cfg.config.Source, cfg.config.WIT, engine.CompileOptions{
...
// buildAsyncImports constructs WIT-style import names for asyncify.
// Format: "namespace.function" (e.g., "wippy:clock.sleep")
func (m *Manager) buildAsyncImports() []string {
	var imports []string
	for _, host := range m.hosts.All() {
		info := host.Info()
		reg := host.Register()
		for funcName := range reg.Functions {
			imports = append(imports, info.Namespace+"."+funcName)
		}
	}
	return imports
}
