// Package wasm provides management for precompiled WASM binary functions.
package wasm
...
// configEntry holds config for a WASM binary function.
type configEntry struct {
	method string
	pool   wasmapi.PoolConfig
	config *wasmapi.WASMFunctionConfig
}
...
// Manager handles precompiled WASM binary function loading, pooling and execution.
type Manager struct {
	ctx        context.Context
	log        *zap.Logger
	bus        event.Bus
...
// NewManager creates a new WASM binary function manager.
func NewManager(
	log *zap.Logger,
	bus event.Bus,
...
func (m *Manager) Add(ctx context.Context, entry registry.Entry) error {
	if entry.Kind != wasmapi.FunctionWASM {
		return runtimewasm.NewInvalidEntryKindError(entry.Kind, wasmapi.FunctionWASM)
	}

	cfg, err := component.UnpackConfig[wasmapi.WASMFunctionConfig](ctx, entry)
	if err != nil {
		return runtimewasm.NewUnpackConfigError("wasm function", err)
	}
...
func (m *Manager) createPool(ctx context.Context, id registry.ID, cfg *configEntry) error {
	wasmBytes, err := component.LoadAndVerifyWASM(m.fsRegistry, cfg.config.FS, cfg.config.Path, cfg.config.Hash)
	if err != nil {
		return err
	}
	...
	module, err := engine.CompileWASM(ctx, m.runtime, wasmBytes, "", engine.CompileOptions{
