...
func (m *Manager) Invalidate(ctx context.Context, ids []registry.ID) {
	...
	for _, id := range ids {
		...
		newFactory := NewActorFactory(frozenBytes, isComponent, old.cfg, m.hostRegistry, m.fsRegistry, m.newCache)
...
func (m *Manager) addWASM(ctx context.Context, entry registry.Entry) error {
	...
	factory := NewActorFactory(frozenBytes, isComponent, cfg, m.hostRegistry, m.fsRegistry, m.newCache)
...
func (m *Manager) updateWASM(ctx context.Context, entry registry.Entry) error {
	...
	newFactory := NewActorFactory(frozenBytes, isComponent, cfg, m.hostRegistry, m.fsRegistry, m.newCache)
	if err := newFactory.warm(ctx); err != nil {
		newFactory.Close()
		return err
	}
