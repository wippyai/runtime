...
func TestActorFactoryHostBufferDomains(t *testing.T) {
	...
	for _, guestPages := range []int64{5, 8} {
		...
		factory := NewActorFactory(code, true, cfg, hosts, nil, component.InMemoryCaches())
		defer factory.Close()
	...
	factory := NewActorFactory(code, true, cfg, component.NewHostRegistry(), nil, component.InMemoryCaches())
	defer factory.Close()
	raw, err := factory.Create()()
	require.NoError(t, err)
	actor := raw.(*engine.ActorProcess)
