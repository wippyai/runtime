...
func init() {
	...
	snapshotType = typ.NewInterface("registry.Snapshot", []typ.Method{
		{Name: "entries", Type: typ.Func().Param("self", typ.Self).Returns(typ.NewArray(typ.Any)).Build()},
		{Name: "get", Type: typ.Func().Param("self", typ.Self).Param("key", typ.String).Returns(typ.Any, typ.NewOptional(typ.LuaError)).Build()},
		{Name: "namespace", Type: typ.Func().Param("self", typ.Self).Param("ns", typ.String).Returns(typ.NewArray(typ.Any)).Build()},
		{Name: "find", Type: typ.Func().Param("self", typ.Self).Param("query", typ.Any).Returns(typ.NewArray(typ.Any)).Build()},
...
func ModuleTypes() *typio.Manifest {
	...
	moduleType := typ.NewInterface("registry", []typ.Method{
		{Name: "get", Type: typ.Func().Param("key", typ.String).Returns(typ.Any, typ.NewOptional(typ.LuaError)).Build()},
		{Name: "find", Type: typ.Func().Param("query", typ.Any).Returns(typ.NewArray(typ.Any), typ.NewOptional(typ.LuaError)).Build()},
