...
better readability. The module also supports field ordering for controlling the structure of output YAML.
...
#### yaml.encode(value: table[, field_order: table])
...
Fields will be ordered based on their position in the `field_order` table. Fields not specified in `field_order` will
appear after the ordered fields.
...
local encoded, err = yaml.encode(myTable)
if err then
  print("Encoding error:", err)
else
  print("Encoded YAML:")
  print(encoded)
  -- Output will include properly formatted multiline text using |
end
...
-- Handle errors
local invalidYaml = "name: Test: Invalid"
local decoded, err = yaml.decode(invalidYaml)
if err then
  print("Error:", err)  -- Output: Error: specific YAML parsing error message
end
