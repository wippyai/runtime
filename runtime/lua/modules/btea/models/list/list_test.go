...
func TestListFiltering(t *testing.T) {
	...
	t.Run("basic filtering", func(t *testing.T) {
		...
		err := vm.DoString(ctx, `
            ...
            -- Test initial state
            assert(list:filtering_enabled(), "filtering should be enabled")
            assert(list:filter_state() == "unfiltered", "should start unfiltered")
            assert(list:filter_value() == "", "should start with empty filter")
            assert(not list:is_filtered(), "should not be filtered initially")
            
            -- Enter filter mode using the slash key
            local enter_filter = {
                type = "key",
                key = {
                    key_type = "runes",
            ...
            list:update(enter_filter)
            assert(list:setting_filter(), "should enter filter mode")

            -- Then type filter text
            local type_filter = {
                type = "key",
                key = {
                    key_type = "runes",
            ...
            list:update(type_filter)
            assert(list:filter_value() == "app", "should update filter value")
            assert(list:is_filtered(), "should be filtered")
            
            -- Check results
            local items = list:items()
            assert(#items == 1, "should filter to one item")
            assert(items[1].title == "Apple", "should match Apple")
        `, "filtering_test")
		require.NoError(t, err)
	})
	...
	t.Run("custom filter function", func(t *testing.T) {
		...
		err := vm.DoString(ctx, `
            ...
            local list = btea.list({
                ...
                items = {
                    { title = "Cat", filter_value = "cat" },
                    { title = "Dog", filter_value = "dog" },
                    { title = "Horse", filter_value = "horse" }
                }
            })
            ...
            local items = list:items()
            assert(#items == 2, "should match two 3-letter words")
        `, "filtering_test")
		require.NoError(t, err)
	})

	t.Run("filter reset", func(t *testing.T) {
		vm := setupListTest(t)
		defer vm.Close()
		...
		err := vm.DoString(ctx, `
            ...
            local list = btea.list({
                ...
                filtering_enabled = true,
                items = {
                    { title = "One", filter_value = "1" },
                    { title = "Two", filter_value = "2" },
                    { title = "Three", filter_value = "3" }
                }
            })
            ...
            assert(list:is_filtered(), "should be filtered")
            assert(#list:items() == 1, "should filter to one item")
            
            -- Reset filter using escape
            list:update({
                type = "key",
                key = {
                    key_type = "esc"
            ...
            assert(not list:is_filtered(), "should not be filtered after reset")
            assert(#list:items() == 3, "should show all items after reset")
            assert(list:filter_value() == "", "should clear filter value")
        `, "filtering_test")
		require.NoError(t, err)
	})
	...
	t.Run("filter state transitions", func(t *testing.T) {
		...
		err := vm.DoString(ctx, `
            ...
            list:update(msg)
            assert(list:setting_filter(), "should enter filter mode")
            assert(list:filter_state() == "filtering", "should be in filtering state")
            
            -- Cancel filtering using escape key
            list:update({
                type = "key",
                key = {
                    key_type = "esc"
            ...
            assert(not list:setting_filter(), "should exit filter mode")
            assert(list:filter_state() == "unfiltered", "should return to unfiltered state")
        `, "filtering_test")
		require.NoError(t, err)
	})
}
