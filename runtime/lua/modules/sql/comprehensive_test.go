...
func TestToGoValueEdgeCases(t *testing.T) {
	tests := []struct {
		...
		{lua.LNumber(0), 0.0, "zero number"},
		{lua.LInteger(0), int64(0), "zero integer"},
		{lua.LNumber(-3.14), -3.14, "negative number"},
		{lua.LInteger(-42), int64(-42), "negative integer"},
		{lua.LInteger(999999), int64(999999), "large integer"},
	}
