...
func init() {
	...
	yieldTypes = []luaapi.YieldType{
		{Sample: &CallYield{}, CmdID: workflowapi.Call},
...
// call executes a child workflow and waits for its result.
...
func call(l *lua.LState) int {
	...
	ctx := l.Context()
	if !security.IsAllowed(ctx, "workflow.call", target, nil) {
	...
	return invalidError(l, "workflow.call outside workflow context not yet supported")
}
