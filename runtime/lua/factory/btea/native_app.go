...
// NativeApp represents a terminal application process with Bubble Tea
...
func (p *NativeApp) Start(ctx context.Context, pid process.PID, input payload.Payloads) error {
	p.ctx = ctx
	p.pid = pid
...
func (p *NativeApp) Send(msg ...*process.Message) error {
	p.mu.Lock()
	defer p.mu.Unlock()
