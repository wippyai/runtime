...
// testHTTPResponseTypes builds an HTTP module manifest with a Response interface.
func testHTTPResponseTypes() *io.Manifest {
	m := io.NewManifest("http")

	responseType := typ.NewInterface("http.Response", []typ.Method{
		{Name: "status", Type: typ.Func().
			Param("self", typ.Self).
			Returns(typ.Integer).
			Build()},
		{Name: "body", Type: typ.Func().
			Param("self", typ.Self).
			Returns(typ.String, typ.NewOptional(typ.LuaError)).
			Build()},
	...
	moduleType := typ.NewInterface("http", []typ.Method{
		{Name: "get", Type: typ.Func().
			...
			Build()},
		{Name: "post", Type: typ.Func().
			Param("url", typ.String).
			OptParam("body", typ.Any).
			Returns(responseType, typ.NewOptional(typ.LuaError)).
			Build()},
	})
...
// testLLMTypes builds an LLM module manifest with union return type.
func testLLMTypes() *io.Manifest {
	m := io.NewManifest("llm")
	...
	resultType := typ.NewUnion(successType, errorType)
	m.DefineType("Result", resultType)

	moduleType := typ.NewInterface("llm", []typ.Method{
		{Name: "complete", Type: typ.Func().
			Param("prompt", typ.String).
			Returns(resultType, typ.NewOptional(typ.LuaError)).
			Build()},
	})
...
// TestFalsePositive_LocalFunctionDBRelease reproduces the pattern where
// a local function wraps sql.get() and the returned DB type loses its
// method signatures, causing E0022 on db:release().
func TestFalsePositive_LocalFunctionDBRelease(t *testing.T) {
	tc := NewTypeChecker(TypeCheckConfig{Enabled: true, Strict: true}, nil)

	imports := map[string]*io.Manifest{
		"sql": sqlmod.ModuleTypes(),
	}
...
`
	...
	for _, d := range diagnostics {
		if d.Severity == diag.SeverityError {
			...
			t.Logf("Error at line %d: [%v] %s", d.Position.Line, d.Code, d.Message)
		}
	}
	assert.Empty(t, errors, "db:query() and db:release() should be valid on wrapped sql.DB")
}

// TestFalsePositive_LocalFunctionDBTransaction reproduces the pattern where
// db:begin() returns a Transaction through a wrapper, and subsequent
// tx:commit()/tx:rollback() calls produce E0022.
func TestFalsePositive_LocalFunctionDBTransaction(t *testing.T) {
	tc := NewTypeChecker(TypeCheckConfig{Enabled: true, Strict: true}, nil)
...
local rows, qerr = tx:query("SELECT 1")
tx:commit()
db:release()
`
	...
	assert.Empty(t, errors, "tx:query(), tx:commit(), db:release() should be valid through wrappers")
}

// TestFalsePositive_CannotIndexNil reproduces E0000 "cannot index nil"
// when accessing fields on a value returned through a wrapper function.
func TestFalsePositive_CannotIndexNil(t *testing.T) {
	tc := NewTypeChecker(TypeCheckConfig{Enabled: true, Strict: true}, nil)

	imports := map[string]*io.Manifest{
		"http": testHTTPResponseTypes(),
	}
...
`
	...
	assert.Empty(t, errors, "resp:status() and resp:body() should be valid on wrapped http.Response")
}

// TestFalsePositive_UnionNarrowingFieldAccess reproduces E0004 where
// a union type {success: true, text: string} | {success: false, error: string}
// is not narrowed after checking the discriminant field.
func TestFalsePositive_UnionNarrowingFieldAccess(t *testing.T) {
	tc := NewTypeChecker(TypeCheckConfig{Enabled: true, Strict: true}, nil)
...
local result, err = llm:complete("hello")
if err then
    error(err:message())
end

if result.success then
    local text: string = result.text
else
    local msg: string = result.error
end
`
	...
	assert.Empty(t, errors, "union narrowing on discriminant field should allow branch-specific field access")
}

// TestFalsePositive_DirectModuleCallWorks verifies that calling module
// methods directly (without wrapper) produces no false positives.
func TestFalsePositive_DirectModuleCallWorks(t *testing.T) {
	tc := NewTypeChecker(TypeCheckConfig{Enabled: true, Strict: true}, nil)
...
`
	...
	assert.Empty(t, errors, "direct sql:get() followed by db:query() and db:release() should work")
}

// TestFalsePositive_NestedWrapperChain reproduces false positives from
// multi-layer function wrappers where type information is lost at each level.
...
`
	...
	assert.Empty(t, errors, "methods should work through nested wrapper chain")
}

// TestFalsePositive_MultiReturnErrorPattern reproduces the common Lua pattern
// where a function returns (value, error) and the caller checks the error
// before using the value.
func TestFalsePositive_MultiReturnErrorPattern(t *testing.T) {
	tc := NewTypeChecker(TypeCheckConfig{Enabled: true, Strict: true}, nil)
...
local function safe_get(url: string)
    local resp, err = http:get(url)
    if err then
        return nil, err
    end
...
local resp, err = safe_get("https://example.com")
if err then
...
`
	...
	assert.Empty(t, errors, "error-checked multi-return should narrow value to non-nil")
}
