...
`
	...
	if isNilType, ok := core.FieldOrMethod(assertManifest.Export, "is_nil"); ok {
	...
	if notNilType, ok := core.FieldOrMethod(assertManifest.Export, "not_nil"); ok {
...
`
	...
	if fn, ok := fetchManifest.Export.(*typ.Function); ok {
		...
		if fn.Spec != nil {
			if spec, ok := fn.Spec.(*contract.Spec); ok {
				t.Logf("fetch_user Spec.Ensures: %v (count: %d)", spec.Ensures, len(spec.Ensures.AllConstraints()))
			} else {
...
`
	...
	if eqType, ok := core.FieldOrMethod(assertManifest.Export, "eq"); ok {
