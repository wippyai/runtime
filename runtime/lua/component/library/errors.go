...
func NewUnpackConfigError(cause error) *Error {
	return &Error{
		kind:      apierror.KindInvalid,
		message:   "failed to unpack library config",
		retryable: apierror.False,
		cause:     cause,
	}
}

func NewAddLibraryNodeError(cause error) *Error {
	return &Error{
...
func NewUpdateLibraryNodeError(cause error) *Error {
	return &Error{
		kind:      apierror.KindInternal,
		message:   "failed to update library node",
		retryable: apierror.False,
		cause:     cause,
	}
}

func NewDeleteLibraryNodeError(cause error) *Error {
	return &Error{
		kind:      apierror.KindInternal,
		message:   "failed to delete library node",
		retryable: apierror.False,
		cause:     cause,
	}
}
