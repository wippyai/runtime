...
var (
	...
	ErrInvalidMaxMutationChanges = apierror.New(apierror.Invalid, "max mutation changes must be non-negative").WithRetryable(apierror.False)
	ErrInvalidMaxMutationBytes   = apierror.New(apierror.Invalid, "max mutation bytes must be non-negative").WithRetryable(apierror.False)
)
