...
var (
	...
	ErrEmptyOptionsEnvKey   = apierror.New(apierror.Invalid, "options_env key must not be empty").WithRetryable(apierror.False)
	ErrEmptyOptionsEnvValue = apierror.New(apierror.Invalid, "options_env value must reference an environment variable").WithRetryable(apierror.False)
)
