...
function file_repo.add_chunk(file_id, section_id, content, chunk_type, path, embedding)
    ...
    -- Only add embedding if it exists and is not nil
    if embedding and embedding ~= "" then
        table.insert(params, embedding)
    else
        -- Return early if embedding is required but missing
        db:release()
        return nil, "Failed to add file chunk: embedding is required"
    end
