...
local function run()
    ...
    local handlers = {
        ...
        __on_event = function(state, event)
            if event.kind == process.event.EXIT then
                ...
                local from_pid = event.event and event.event.from
                if from_pid then
        ...
        [WS_JOIN_TOPIC] = function(state, payload)
            --if type(payload) == "table" and payload.user_id then
            --    -- Remove this hub from our tracking
            --    if state.user_hubs[payload.user_id] then
            --        print("User hub for", payload.user_id, "is exiting")
            --        state.user_hubs[payload.user_id] = nil
            --        state.total_hubs = state.total_hubs - 1
            --    end
            --end
    ...
    --        targetPID = user_hub_pid,
    --        messageTopic = WS_MESSAGE_TOPIC,
    --        metadata = {
    --            user_id = user_id,
    --            redirected_at = time.now():format_rfc3339()
    --        }
    --    })
    --
    --    -- Update stats
    ...
    --        -- Check if the existing hub is still alive
    --        local exists, err = process.send(state.user_hubs[user_id].hub_pid, PING_TOPIC)
    --        if exists then
    --            -- Update last activity time
    --            state.user_hubs[user_id].last_activity = time.now()
    --            return state.user_hubs[user_id].hub_pid
    --        end
    --        -- If send failed, the process is dead, clean up and create a new one
    --        print("User hub for", user_id, "is not responding, creating a new one")
    --        state.user_hubs[user_id] = nil
    --    end
    --
