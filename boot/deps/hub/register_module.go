...
func (c *Client) RegisterModule(ctx context.Context, p *RegisterModuleParams) (*RegisteredModule, error) {
	...
	if err != nil {
		return nil, NewHubRequestError("marshal register-module request", err)
	}

	req, err := http.NewRequestWithContext(ctx, http.MethodPost,
		c.baseURL+"/api/v1/account/modules", bytes.NewReader(body))
	if err != nil {
		return nil, NewHubRequestError("build register-module request", err)
	}
	...
	if err != nil {
		return nil, NewHubRequestError("post register-module", err)
	}
	...
	case http.StatusCreated, http.StatusOK:
		...
		if err := json.Unmarshal(respBody, &out); err != nil {
			return nil, NewHubRequestError("decode register-module response", err)
		}
		return &out, nil
	case http.StatusConflict:
		return nil, ErrModuleAlreadyExists
	default:
		return nil, NewHubResponseError("register-module", resp.StatusCode, string(respBody))
	}
}
