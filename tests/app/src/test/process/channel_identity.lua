...
local function main()
    ...
    worker_pid, err = process.spawn_monitored("app.test.process:send_and_exit_worker", "app:processes", process.pid())
    if err then
    ...
    worker_pid, err = process.spawn_monitored("app.test.process:send_custom_worker", "app:processes", process.pid(), "custom_select_topic")
    if err then
    ...
    worker_pid, err = process.spawn_monitored("app.test.process:instant_exit_worker", "app:processes")
    if err then
    ...
    local matched = false
    if result.channel == final_inbox then matched = true end
    if result.channel == final_events then matched = true end
    if result.channel == final_listen then matched = true end
