...
local function main()
    ...
    local ok, send_err = process.send(worker_pid, "test_topic", "msg1")
    ...
    ok, send_err = process.send(worker_pid, "test_topic", "msg2")
    assert.is_nil(send_err, "send msg2 no error")

    -- Signal worker to finish via inbox
    ok, send_err = process.send(worker_pid, "inbox", "done")
