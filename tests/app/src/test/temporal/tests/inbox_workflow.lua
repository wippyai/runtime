...
local function main()
	...
	assert.is_nil(err, "spawn inbox workflow no error")
	assert.is_string(pid, "got pid")

	local ok, send_err = process.send(pid, "ping", { hello = "world" })
	assert.is_nil(send_err, "send to inbox workflow")
	...
	assert.eq(event.result.value.received_topic, "ping", "received topic")
	assert.is_table(event.result.value.received_payload, "received payload table")
	assert.eq(event.result.value.received_payload.hello, "world", "payload field")
