...
local function main()
	...
	local ok_run, run_err = pcall(function()
		local workflow_id = "startup-explicit-id-error-on-started-" .. tostring(time.now():unix_nano())
		...
		local second = process
			...
			:with_message("increment", { amount = 9 })
		...
		assert.is_nil(pid2, "error_when_already_started returns nil pid")
		assert.not_nil(err2, "error_when_already_started returns error")
		if tostring(err2) == "" then
			error("already started path should include non-empty error message")
		end
