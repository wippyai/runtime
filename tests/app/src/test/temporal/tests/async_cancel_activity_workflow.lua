...
local function main()
	...
	local workflow_name = "async-cancel-" .. tostring(time.now():unix_nano())
	local pid, err = process
		.with_options({})
		:with_name(workflow_name)
		:spawn_monitored(
			"app.test.temporal.workflows:async_cancel_activity_workflow",
	...
	assert.eq(result.status, "canceled", "cancel status")
	assert.eq(result.canceled, true, "future marked canceled")
	assert.contains(tostring(result.future_error), "canceled", "future returns canceled error")
