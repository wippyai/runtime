...
local function main()
	...
	local ok_run, run_err = pcall(function()
		local workflow_id = "startup-explicit-id-conflict-fail-" .. tostring(time.now():unix_nano())
		...
		local second = process
			...
			:with_message("increment", { amount = 7 })
		...
		assert.is_nil(pid2, "conflict fail returns nil pid")
		assert.not_nil(err2, "conflict fail returns error")
		if tostring(err2) == "" then
			error("conflict fail should include non-empty error message")
		end
