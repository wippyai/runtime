...
local function main(input)
    ...
    local length, err = workflow.history_length()
    results.history_length = length

    -- Test 3: workflow.history_size() should return >= 0
    local size, err = workflow.history_size()
    ...
    -- Test 5: workflow.call() to execute child workflow
    local child_result, err = workflow.call("app.test.temporal:child_workflow", {
        message = "hello from parent"
    })
    if err then
        results.call_error = tostring(err)
    else
        results.call_result = child_result
    end

    -- Test 6: Multiple version calls with same change_id should return same version
    local version2, err = workflow.version("test_change_v1", 1, 3)
    results.version_consistent = (version == version2)

    -- Test 7: Different change_id can have different version
    local version3, err = workflow.version("test_change_v2", 0, 5)
