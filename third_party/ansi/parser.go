...
type Parser struct {
	...
	// utf8Remain counts the UTF-8 continuation bytes still expected while
	// string data is being collected. It disambiguates a 0x9C that continues
	// a rune (string data) from the 8-bit ST control (string terminator).
	utf8Remain int
}
...
// isUtf8StringState reports whether the parser is collecting string data whose
// bytes may be arbitrary UTF-8. Only OSC and DCS extend their transition table
// to the full 0x80-0xFF byte range; SOS, PM and APC remain byte-oriented.
func (p *Parser) isUtf8StringState() bool {
	return p.state == parser.OscStringState || p.state == parser.DcsStringState
}
...
func (p *Parser) advance(b byte) parser.Action {
	// OSC and DCS strings carry UTF-8 payloads, but the DEC transition table
	// also treats 0x9C as the 8-bit String Terminator. When 0x9C is a UTF-8
	// continuation byte it is data: without this, the 0x9C inside a character
	// such as U+2733 (E2 9C B3) terminates the string and the remaining bytes
	// are printed to the screen. A genuine 8-bit ST, which does not continue a
	// rune, still terminates the string.
	stringData := false
	if p.isUtf8StringState() {
		if p.utf8Remain > 0 && b >= 0x80 && b <= 0xBF {
			p.utf8Remain--
			stringData = b == 0x9C
		} else {
			switch {
		...
		// Never let a truncated rune leak a pending count into a later string.
		p.utf8Remain = 0
	}
