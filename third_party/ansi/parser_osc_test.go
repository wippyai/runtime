...
func TestOscSequence(t *testing.T) {
	...
	cases := []testCase{
		...
		{
			// U+672C is E6 9C AB. The 0x9C is a UTF-8 continuation byte, not
			// the 8-bit ST, so the payload is collected whole and the string
			// ends at ESC \\. Verified against tmux, which reports the title
			// as U+672C.
			name:  "utf8_rune_is_not_string_terminator",
