...
func TestDispatcher_Query(t *testing.T) {
	...
	_, err := db.ExecContext(context.Background(), `INSERT INTO test (name) VALUES ('alice'), ('bob')`)
	require.NoError(t, err)
...
func TestDispatcher_Transaction(t *testing.T) {
	...
	var count int
	err = db.QueryRowContext(context.Background(), "SELECT COUNT(*) FROM test WHERE name = 'dave'").Scan(&count)
	require.NoError(t, err)
...
func TestDispatcher_ContextCancellation(t *testing.T) {
	...
	// Should not complete because context was canceled
	select {
	case <-completed:
		t.Fatal("should not complete when context is canceled")
