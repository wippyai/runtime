...
func TestPerPoolObserverCapturesOwnDatabase(t *testing.T) {
	...
	for _, db := range []*openedDBForTest{first, second} {
		_, err := db.opened.DB.Exec(`CREATE TABLE items (id INTEGER PRIMARY KEY, value TEXT)`)
		require.NoError(t, err)
	}
	...
	_, err = first.opened.DB.Exec(`INSERT INTO items (id, value) VALUES (1, 'first')`)
	require.NoError(t, err)
	_, err = second.opened.DB.Exec(`INSERT INTO items (id, value) VALUES (2, 'second')`)
	require.NoError(t, err)
...
func TestObserverRebindsAfterConnectionExpiry(t *testing.T) {
	...
	_, err = observed.opened.DB.Exec(`CREATE TABLE items (id INTEGER PRIMARY KEY, value TEXT)`)
	require.NoError(t, err)
	...
	_, err = observed.opened.DB.Exec(`INSERT INTO items (id, value) VALUES (1, 'rebound')`)
	require.NoError(t, err)
...
func TestObserverCloseCancelsSnapshotRead(t *testing.T) {
	...
	require.NoError(t, err)
	for i := 1; i <= 64; i++ {
		_, err = observed.opened.DB.Exec(`INSERT INTO items (id, value) VALUES (?, ?)`, i, "value")
		require.NoError(t, err)
	}
...
func TestObserverPublishesNetTransactionAfterCommit(t *testing.T) {
	...
	tx, err := observed.opened.DB.Begin()
	require.NoError(t, err)
	_, err = tx.Exec(`INSERT INTO items (id, value) VALUES (1, 'first')`)
	require.NoError(t, err)
	_, err = tx.Exec(`UPDATE items SET value = 'second' WHERE id = 1`)
	require.NoError(t, err)
	_, err = tx.Exec(`UPDATE items SET value = 'final' WHERE id = 1`)
	require.NoError(t, err)
	...
	tx, err = observed.opened.DB.Begin()
	require.NoError(t, err)
	_, err = tx.Exec(`INSERT INTO items (id, value) VALUES (2, 'transient')`)
	require.NoError(t, err)
	_, err = tx.Exec(`DELETE FROM items WHERE id = 2`)
	require.NoError(t, err)
	require.NoError(t, tx.Commit())
	select {
...
func TestObserverSavepointRollbackDoesNotPublishRolledBackRows(t *testing.T) {
	...
	require.NoError(t, err)
	_, err = tx.Exec(`SAVEPOINT /* comment */ nested`)
	require.NoError(t, err)
	_, err = tx.Exec(`INSERT INTO items (id, value) VALUES (2, 'rolled-back')`)
	require.NoError(t, err)
	_, err = tx.Exec(`ROLLBACK /* comment */ TO SAVEPOINT nested`)
	require.NoError(t, err)
	_, err = tx.Exec(`RELEASE /* comment */ SAVEPOINT nested`)
	require.NoError(t, err)
	require.NoError(t, tx.Commit())
...
func TestObserverFailsClosedForAmbiguousPartialStatement(t *testing.T) {
	...
	_, err = observed.opened.DB.Exec(`CREATE TABLE items (id INTEGER PRIMARY KEY, value TEXT UNIQUE)`)
	require.NoError(t, err)
	stream, err := observed.opened.Observer.Subscribe(context.Background(), config.MutationOptions{})
	require.NoError(t, err)
	defer func() { _ = stream.Close() }()
	...
	tx, err := observed.opened.DB.Begin()
	require.NoError(t, err)
	_, err = tx.Exec(`INSERT OR FAIL INTO items (id, value) VALUES (1, 'first'), (2, 'first')`)
	require.Error(t, err)
	require.NoError(t, tx.Commit())
...
func TestObserverSnapshotFencesLiveWrites(t *testing.T) {
	...
	require.NoError(t, err)
	for i := 1; i <= 3; i++ {
		_, err = observed.opened.DB.Exec(`INSERT INTO items (id, value) VALUES (?, ?)`, i, "old")
		require.NoError(t, err)
	}
	...
	go func() {
		_, writeErr := observed.opened.DB.Exec(`INSERT INTO items (id, value) VALUES (99, 'live')`)
		writeDone <- writeErr
	}()
...
func TestObserverSnapshotHandoffIncludesInFlightWriterAsLive(t *testing.T) {
	...
	tx, err := observed.opened.DB.Begin()
	require.NoError(t, err)
	_, err = tx.Exec(`INSERT INTO items (id, value) VALUES (10, 'in-flight')`)
	require.NoError(t, err)
...
func TestObserverAbortedStatementDoesNotPublishPartialRows(t *testing.T) {
	...
	tx, err := observed.opened.DB.Begin()
	require.NoError(t, err)
	_, err = tx.Exec(`INSERT INTO items (id, value) VALUES (1, 'same'), (2, 'same')`)
	require.Error(t, err)
	require.NoError(t, tx.Commit())
	select {
	case _, ok := <-stream.Changes():
...
func TestObserverFailsClosedWithoutSQLConflictInference(t *testing.T) {
	...
	tx, err := observed.opened.DB.Begin()
	require.NoError(t, err)
	_, err = tx.Exec(`INSERT /* OR FAIL */ INTO items (id, value) VALUES (1, 'literal'), (2, 'literal')`)
	require.Error(t, err)
	require.NoError(t, tx.Commit())
	select {
	case _, ok := <-stream.Changes():
...
func TestObserverFailedSavepointCommandDoesNotCorruptCapture(t *testing.T) {
	...
	_, err = tx.Exec(`INSERT INTO items (id, value) VALUES (1, 'kept')`)
	require.NoError(t, err)
	_, err = tx.Exec(`ROLLBACK TO SAVEPOINT missing`)
	require.Error(t, err)
	require.NoError(t, tx.Commit())
	batch := receiveBatch(t, stream)
	require.Len(t, batch.Changes, 1)
...
func TestObserverFailsClosedForMultipleSavepointsInOneExec(t *testing.T) {
	...
	stream, err := observed.opened.Observer.Subscribe(context.Background(), config.MutationOptions{})
	require.NoError(t, err)
	_, err = observed.opened.DB.Exec(`SAVEPOINT first; SAVEPOINT second`)
	require.NoError(t, err)
	select {
	case _, ok := <-stream.Changes():
		require.False(t, ok)
...
func TestObserverReturningRowsFinalizeOnClose(t *testing.T) {
	...
	require.NoError(t, err)
	defer func() { _ = stream.Close() }()
	rows, err := observed.opened.DB.Query(`INSERT INTO items (id, value) VALUES (1, 'returned') RETURNING id`)
	require.NoError(t, err)
	var id int64
...
func TestObserverFiltersLiveMutations(t *testing.T) {
	...
	require.NoError(t, err)
	defer observed.Close()
	_, err = observed.opened.DB.Exec(`CREATE TABLE items (id INTEGER PRIMARY KEY, value TEXT); CREATE TABLE ignored (id INTEGER PRIMARY KEY, value TEXT)`)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	defer func() { _ = stream.Close() }()
	_, err = observed.opened.DB.Exec(`INSERT INTO ignored (id, value) VALUES (1, 'no')`)
	require.NoError(t, err)
	_, err = observed.opened.DB.Exec(`INSERT INTO items (id, value) VALUES (2, 'yes')`)
	require.NoError(t, err)
	batch := receiveBatch(t, stream)
	require.Len(t, batch.Changes, 1)
...
func TestObserverOverflowClosesWithoutBlockingCommit(t *testing.T) {
	...
	require.NoError(t, err)
	_, err = observed.opened.DB.Exec(`INSERT INTO items (id, value) VALUES (1, 'one'), (2, 'two')`)
	require.NoError(t, err)
	select {
	case _, ok := <-stream.Changes():
		require.False(t, ok)
...
func TestObserverKeepsMultiStatementCommitBoundaries(t *testing.T) {
	...
	require.NoError(t, err)
	defer func() { _ = stream.Close() }()
	_, err = observed.opened.DB.Exec(`INSERT INTO items (id, value) VALUES (?,'one'); INSERT INTO items (id, value) VALUES (?,'two')`, 1, 2)
	require.NoError(t, err)
...
func TestObserverPublishesEarlierAutocommitBeforeLaterError(t *testing.T) {
	...
	require.NoError(t, err)
	defer func() { _ = stream.Close() }()
	_, err = observed.opened.DB.Exec(`INSERT INTO items (id, value) VALUES (1,'one')`)
	require.NoError(t, err)
	_, err = observed.opened.DB.Exec(`INSERT INTO items (id, value) VALUES (2,'one')`)
	require.Error(t, err)
	batch := receiveBatch(t, stream)
	require.Len(t, batch.Changes, 1)
	assert.Equal(t, int64(1), batch.Changes[0].RowID)
...
func TestObserverRejectsUnsupportedVirtualTable(t *testing.T) {
	...
	require.NoError(t, err)
	defer observed.Close()
	_, err = observed.opened.DB.Exec(`CREATE VIRTUAL TABLE docs USING fts5(content)`)
	if err != nil {
...
func TestObserverDetectsDDLThroughAuthorizerWithLeadingComment(t *testing.T) {
	...
	tx, err := observed.opened.DB.Begin()
	require.NoError(t, err)
	_, err = tx.Exec(`/* CREATE TABLE */ CREATE TABLE other (id INTEGER PRIMARY KEY, value TEXT)`)
	require.NoError(t, err)
	_, err = tx.Exec(`INSERT INTO items (id, value) VALUES (1, 'value')`)
	require.NoError(t, err)
	require.NoError(t, tx.Commit())
	select {
	case _, ok := <-stream.Changes():
