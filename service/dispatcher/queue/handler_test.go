...
func (m *mockManager) Publish(ctx context.Context, queueID registry.ID, msgs ...*queue.Message) error {
	if m.publishErr != nil {
		return m.publishErr
	}
...
func TestQueuePublishHandler(t *testing.T) {
	h := NewQueuePublishHandler()
	mgr := &mockManager{}
...
func TestQueuePublishHandlerError(t *testing.T) {
	h := NewQueuePublishHandler()
