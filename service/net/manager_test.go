...
func TestManager_Update_UnregistersBeforeRecreate(t *testing.T) {
	d := &fakeDriver{kind: "network.fake"}
	m, reg := newTestManager(t, WithDriver(d))
	...
	// Swap the driver to fail: Update must unregister the old instance
	// even when the new Create fails.
	d.createErr = errors.New("refreshed driver refuses")
	_ = m.Update(context.Background(), registry.Entry{ID: id, Kind: "network.fake"})

	assert.False(t, reg.HasNetwork(id), "Update must unregister before attempting to recreate")
}
