...
func NewClientManager(logger *zap.Logger) *Manager {
	return NewClientManagerWithFactory(logger, NewDefaultFactory(), nil)
}

// NewClientManagerWithFactory creates a new client manager with a custom factory
func NewClientManagerWithFactory(logger *zap.Logger, factory Factory, dc converter.DataConverter) *Manager {
	return &Manager{
		log:      logger,
		factory:  factory,
		dc:       dc,
