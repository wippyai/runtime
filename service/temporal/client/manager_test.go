...
func (m *mockEnvRegistry) Has(ctx context.Context, key string) bool {
	_, ok := m.values[key]
	return ok
}
...
func TestManager_AddClient(t *testing.T) {
	t.Run("add valid client config", func(t *testing.T) {
		...
		assert.Len(t, bus.events, 2) // supervisor.Register + resource.Register
	})
...
func TestManager_UpdateClient(t *testing.T) {
	t.Run("update existing client", func(t *testing.T) {
		...
		assert.Len(t, bus.events, 1) // supervisor.Update
		assert.Equal(t, supervisor.Update, bus.events[0].Kind)
	})
...
func TestManager_DeleteClient(t *testing.T) {
	t.Run("delete existing client", func(t *testing.T) {
		...
		assert.Len(t, bus.events, 2) // supervisor.Remove + resource.Delete
	})
