...
func executeTimer(cmd runtime.Command, env bindings.WorkflowEnvironment, _ converter.DataConverter, transcoder payload.Transcoder) error {
	params := cmd.Params()
	if len(params) < 1 {
	...
	// Execute the timer
	env.NewTimer(duration, workflow.TimerOptions{}, func(_ *commonpb.Payloads, err error) {
		if err != nil {
			completeCommand(cmd, nil, err)
			return
		}
