...
func (d *Definition) Execute(env bindings.WorkflowEnvironment, _ *commonpb.Header, input *commonpb.Payloads) {
	...
	// Completion callback
	ctx := process.WithAddedOnComplete(d.ctx, func(_ pubsub.PID, result *runtime.Result) {
		d.result = result
	})
	...
	env.RegisterSignalHandler(func(name string, _ *commonpb.Payloads, _ *commonpb.Header) error {
		log.Printf("SIGNAL! %v", name)
		// d.wfl.Send(pubsub.NewPackage())
		return nil
	})

	env.RegisterQueryHandler(func(name string, _ *commonpb.Payloads, _ *commonpb.Header) (*commonpb.Payloads, error) {
