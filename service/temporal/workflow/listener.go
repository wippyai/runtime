...
// NewListener creates a new listener for Temporal workflows and registers it with the event bus
func NewListener(
	bus event.Bus,
	log *zap.Logger,
) eventbus.EventHandler {
	listener := &Listener{log: log, bus: bus}
	return listener
}
...
func (l *Listener) handleFunctionEntry(ctx context.Context, entry registry.Entry) error {
...
func (l *Listener) handleFunctionDelete(ctx context.Context, entry registry.Entry) error {
	// Check if entry is a workflow type
	if !isWorkflowTarget(entry) {
		return nil
	}
...
func (l *Listener) getTaskQueueID(workflowMeta registry.Metadata, defaultNS string) (registry.ID, error) {
...
func (l *Listener) getMetaDuration(meta registry.Metadata, key string) (time.Duration, error) {
...
func (l *Listener) getWorkflowOptions(workflowMeta registry.Metadata) (*client.StartWorkflowOptions, error) {
	...
	if retryPolicyMeta != nil {
		...
		if maxAttempts := retryPolicyMeta.IntValue(MetaRetryMaximumAttempts); maxAttempts > 0 {
			if maxAttempts <= int(^uint32(0)>>1) {
				retryPolicy.MaximumAttempts = int32(maxAttempts)
			} else {
				return nil, fmt.Errorf("maxAttempts value %d overflows int32", maxAttempts)
			}
...
func (l *Listener) getWakeUpSignals(workflowMeta registry.Metadata) []string {
