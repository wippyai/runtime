...
// A rename that succeeds ahead of a failing durability sync reports the
// publication as durable-uncertain rather than as a failed write.
func TestAtomicWriteReportsUncertainDurabilityWhenSyncFails(t *testing.T) {
	dir := t.TempDir()
	d := newAtomicFS(t, dir, 0700)

	fixture := errors.New("sync refused")
	original := atomicSyncPublication
	atomicSyncPublication = func(*os.Root, string) error { return fixture }
	t.Cleanup(func() { atomicSyncPublication = original })
