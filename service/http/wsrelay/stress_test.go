...
func TestSecurity_MetadataInjection(t *testing.T) {
	testCases := []struct {
		...
		{"null value", map[string]any{"key": nil}},
		{"nested object", map[string]any{"nested": map[string]any{"deep": "value"}}},
		{"array value", map[string]any{"arr": []int{1, 2, 3}}},
		{"special chars", map[string]any{"key": "<script>alert('xss')</script>"}},
		{"sql injection", map[string]any{"key": "'; DROP TABLE users; --"}},
		{"path traversal", map[string]any{"key": "../../../etc/passwd"}},
		{"large string", map[string]any{"key": repeatString("a", 100000)}},
	}
