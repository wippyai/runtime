...
func TestExporter_ConcurrentRecords(t *testing.T) {
	...
	body := rec.Body.String()
	assert.Contains(t, body, "concurrent_counter")
	assert.Contains(t, body, "concurrent_gauge")
}
...
func TestBuildMetricKey(t *testing.T) {
	tests := []struct {
		...
		{"no labels", "metric", "metric", nil},
		{"empty labels", "metric", "metric", []string{}},
		{"one label", "metric", "metric{env}", []string{"env"}},
		{"multiple labels", "metric", "metric{env,host}", []string{"env", "host"}},
	}
