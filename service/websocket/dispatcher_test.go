...
func readOnlyServer(_ *testing.T) *httptest.Server {
	return httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		conn, err := websocket.Accept(w, r, nil)
		if err != nil {
			return
...
func TestNoRegistryErrors(t *testing.T) {
	...
	testCases := []struct {
		...
		{wsapi.SendCmd{ConnID: 1, Data: []byte("x")}, "send", wsapi.Send},
		{wsapi.ReceiveCmd{ConnID: 1}, "receive", wsapi.Receive},
		{wsapi.CloseCmd{ConnID: 1}, "close", wsapi.Close},
		{wsapi.PingCmd{ConnID: 1}, "ping", wsapi.Ping},
	}
