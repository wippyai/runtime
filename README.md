  ...
  workspace:
    workspace:
      ...
      replacement_excludes:
        wippy/runtime:
          - wippy.runtime:onboarding
          - wippy.runtime.instrument:**
```

`replacement_excludes` is optional. Its entry-ID or namespace patterns select
only the local runtime surface needed by the host, so onboarding or instrument
entries in a replacement do not boot with the application.
