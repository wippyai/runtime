...
func runApplication(ctx context.Context, options Options, stateDir string, base bool, command string, remaining []string, owner OwnerOptions) (result error) {
	...
	skipped, err := seedDependencyCache(stateDir, deployment, options.Bundle)
	if err != nil {
		return err
	}
	for _, failure := range skipped {
		// A retained deployment only shortens startup. The launch continues on
		// the deployments that remain readable, and each skipped source is named
		// so the state directory can be repaired.
		fmt.Fprintf(os.Stderr, "WARNING: retained deployment skipped while seeding the dependency cache: %v\n", failure)
	}
	...
	overrides = bootconfig.Merge(launchOverrides(overrides, historyPath), boot.NewConfig(boot.WithSection("registry", map[string]any{
