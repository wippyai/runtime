...
func main() {
	...
	startMassiveWorkflows(ctx, c, workerTaskQueue)
...
func startMassiveWorkflows(ctx context.Context, c client.Client, taskQueue string) {
	...
	for batch := 0; batch < totalRuns/batchSize; batch++ {
		select {
		case <-ctx.Done():
			log.Println("Context canceled, stopping workflow execution")
			return
		case <-rateLimiter:
