...
func TestUniqIDGenerator(t *testing.T) {
	...
	t.Run("format at boundaries", func(t *testing.T) {
		tests := []struct {
			...
			{0, "0x00001"},                     // first ID (5 digits)
			{0xFFFFE, "0xfffff"},               // 5 digits max
			{0xFFFFF, "0x100000"},              // 6 digits start
			{0xFFFFFF, "0x01000000"},           // 8 digits start
			{0xFFFFFFFF, "0x0000000100000000"}, // 16 digits start
		}
