...
build-runner-local:
	mkdir -p ./dist
	CGO_ENABLED=1 go build -o ./dist/runner-$(shell go env GOOS)-$(shell go env GOARCH) ./cmd/runner/main.go
...
build-runner-linux-amd64:
	mkdir -p ./dist
	CGO_ENABLED=1 GOOS=linux GOARCH=amd64 go build -o ./dist/runner-linux-amd64 ./cmd/runner/main.go

build-runner-linux-arm64:
	mkdir -p ./dist
	CGO_ENABLED=1 GOOS=linux GOARCH=arm64 CC=aarch64-linux-gnu-gcc go build -o ./dist/runner-linux-arm64 ./cmd/runner/main.go

build-runner-darwin-amd64:
	mkdir -p ./dist
	CGO_ENABLED=1 GOOS=darwin GOARCH=amd64 CC=o64-clang go build -o ./dist/runner-darwin-amd64 ./cmd/runner/main.go

build-runner-darwin-arm64:
	mkdir -p ./dist
	CGO_ENABLED=1 GOOS=darwin GOARCH=arm64 CC=oa64-clang go build -o ./dist/runner-darwin-arm64 ./cmd/runner/main.go

build-runner-windows-amd64:
	mkdir -p ./dist
	CGO_ENABLED=1 GOOS=windows GOARCH=amd64 CC=x86_64-w64-mingw32-gcc go build -o ./dist/runner-windows-amd64.exe ./cmd/runner/main.go