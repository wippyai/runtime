...
Result {
    ...
    runtime: number       # Process runtime in seconds
}
```
...
local cmd = process.new(name, args)
...
cmd:set_options({
    ...
    idle_timeout = number,
    io_config = {
        collect_stdout = boolean,
        collect_stderr = boolean,
        max_buffer = number
    }
})
```
...
-- Read all collected output (requires collect_stdout/stderr = true)
...
local cmd = process.new("python", {"script.py"})
cmd:set_options({
    work_dir = "/scripts",
    env = {
...
local success, err = cmd:start()
if not success then
    error("Failed to start process: " .. err)
end
...
if result.exit_code ~= 0 then
    error("Process failed: " .. result.error)
end

print("Process completed in " .. result.runtime .. " seconds")
```