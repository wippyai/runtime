    ...
    describe("Claude Text Generation Handler", function()
            ...
            mock(claude_client, "send_request", function(self, endpoint_path, payload, options)
                ...
                return {
                    content = {
                        {
                            type = "text",
                            text = "This response was truncated due to max_tokens limit"
                        }
                    },
                    id = "msg_mock456",
                    model = "claude-3-5-haiku-20241022",
        ...
        it("should handle developer messages correctly with mocked client", function()
            ...
            mock(claude_client, "send_request", function(self, endpoint_path, payload, options)
                ...
                expect(#payload.messages >= 3).to_be_true("Expected at least 3 messages")

                -- Find the developer content (it should be an assistant message)
                local found_dev_content = false
                for _, msg in ipairs(payload.messages) do
                    if msg.role == "assistant" and msg.content[1] and
                        msg.content[1].text and msg.content[1].text:match("concise answer") then
                        found_dev_content = true
                        break
                    end
                end

                expect(found_dev_content).to_be_true("Didn't find developer message content")
        ...
        it("should handle cache control with real API", function()
            ...
            -- If cache was created, cache_creation_input_tokens should be non-zero
