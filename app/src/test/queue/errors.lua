...
local function main()
    ...
    -- Test error for missing queue ID
    ...
    assert.is_nil(ok, "missing data should return nil")
    assert.not_nil(err, "missing data should return error")
    assert.eq(err:kind(), errors.INVALID, "missing data error kind should be INVALID")
