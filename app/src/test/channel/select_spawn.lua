...
local function main()
    ...
    coroutine.spawn(function()
        ...
        local which = 0
        if result.channel == ch1 then which = 1 end
        if result.channel == ch2 then which = 2 end
        result_ch:send({
            which = which,
            value = result.value,
            ok = result.ok
    ...
    coroutine.spawn(function()
        ...
        local which = 0
        if result.channel == ch3 then which = 3 end
        if result.channel == ch4 then which = 4 end
        result_ch2:send({
            which = which,
            ok = result.ok
        })
