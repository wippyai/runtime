...
func (s *Scheduler) getHandler(cmd Command) Handler {
	if s.dispatcher != nil {
...
// The process is initialized with Start() and queued for execution.
func (s *Scheduler) Submit(ctx context.Context, pid relay.PID, p Process, input payload.Payloads) (*Processor, error) {
...
func (s *Scheduler) Execute(ctx context.Context, pid relay.PID, p Process, input payload.Payloads) (*runtime.Result, error) {
	if err := p.Start(ctx, input); err != nil {
		return nil, err
	}
...
func (s *Scheduler) completeProcessor(proc *Processor, yields []Command, err error) {
	// Unregister from lookups FIRST (before any cleanup)
	...
	// Notify blocking Execute() caller
