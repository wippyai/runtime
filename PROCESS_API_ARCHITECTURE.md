...
**API Level** - `wippy/api/process/process.go`
...
**System Level** - `wippy/system/process/manager.go`
...
**Service Level (Managed Host)** - `wippy/service/host/host.go`
...
**Runtime Level (Lua Process)** - `wippy/runtime/lua/component/process/process.go`
...
Stored in `wippy/api/service/host/config.go`:
...
Defined in `wippy/api/runtime/task.go`:
...
**Retry Interceptor** - `wippy/api/service/interceptor/retry/config.go`:
...
**OpenTelemetry Interceptor** - `wippy/api/service/interceptor/otel/config.go`:
...
**Process Execution** - `wippy/api/service/exec/api.go`:
...
**Base Interface** - `wippy/api/process/process.go`:
...
**In HostRegistry** - `wippy/system/process/host_registry.go`:
...
**Managed Host** - `wippy/service/host/host.go`:
...
- `wippy/api/process/process.go` - Main interfaces (Start, Launch, Process, Host)
- `wippy/api/process/context.go` - Lifecycle callbacks
- `wippy/api/runtime/task.go` - Task structure with Options

### System Layer
- `wippy/system/process/manager.go` - Start orchestration
- `wippy/system/process/host_registry.go` - Host lookup
- `wippy/system/process/prototype_registry.go` - Process creation

### Service Layer
- `wippy/service/host/host.go` - Managed host implementation
- `wippy/service/host/pool.go` - Process pool (150+ lines)
- `wippy/service/host/factory.go` - Host creation
- `wippy/api/service/host/config.go` - Host configuration

### Runtime Layer
- `wippy/runtime/lua/component/process/process.go` - Lua process entry point
- `wippy/runtime/lua/component/process/manager.go` - Lua manager
- `wippy/runtime/lua/component/process/state.go` - Lua state management

### Bootstrap Layer
- `wippy/boot/components/service/service/host.go` - Host component bootstrap
- `wippy/boot/components/system/system/functions.go` - Function registry bootstrap
- `wippy/boot/components/service/service/all.go` - Interceptor registration

### Configuration Examples
- `wippy/api/service/interceptor/retry/config.go` - Config duplication example
- `wippy/api/service/interceptor/otel/config.go` - OTEL options
- `wippy/runtime/lua/code/build_options.go` - BuildOptions (different pattern)
