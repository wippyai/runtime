...
// watcher implements cluster.Watcher using the event bus.
type watcher struct {
...
func newWatcher(ctx context.Context, bus event.Bus, system event.System, prefix string) (*watcher, error) {
	...
	watchCh := make(chan cluster.WatchEvent, 64)
...
func (w *watcher) deliver(source <-chan event.Event, prefix string) {
	...
	for {
		...
		case evt, ok := <-source:
			...
			watchEvt, ok := evt.Data.(cluster.WatchEvent)
			if !ok {
				continue
			}
...
func (w *watcher) Events() <-chan cluster.WatchEvent {
	return w.events
}
...
var _ cluster.Watcher = (*watcher)(nil)
