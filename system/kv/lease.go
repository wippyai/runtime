...
// lease implements cluster.Lease.
type lease struct {
...
func newLease(id cluster.LeaseID, ttl time.Duration) *lease {
	return &lease{
		id:   id,
		ttl:  ttl,
...
func (l *lease) ID() cluster.LeaseID   { return l.id }
...
type leaseManager struct {
	heap    leaseHeap
	entries map[cluster.LeaseID]*leaseHeapEntry
	handles map[cluster.LeaseID]*lease
}

func newLeaseManager() *leaseManager {
	return &leaseManager{
		entries: make(map[cluster.LeaseID]*leaseHeapEntry),
		handles: make(map[cluster.LeaseID]*lease),
	}
}

// grant creates a new lease with the given TTL.
func (m *leaseManager) grant(id cluster.LeaseID, ttl time.Duration, now time.Time) *lease {
...
func (m *leaseManager) renew(id cluster.LeaseID, now time.Time) bool {
...
func (m *leaseManager) revoke(id cluster.LeaseID) bool {
	entry, ok := m.entries[id]
	if !ok {
		return false
	}
...
func (m *leaseManager) expired(now time.Time) []cluster.LeaseID {
	var result []cluster.LeaseID
	for m.heap.Len() > 0 {
		top := m.heap[0]
...
func (m *leaseManager) getHandle(id cluster.LeaseID) (*lease, bool) {
	h, ok := m.handles[id]
	return h, ok
}
