...
//
//	op:1 | keyLen:4 | key | valLen:4 | val | expect:8 | leaseLen:4 | lease | ttl:8 | expiresAt:8
func encodeCommand(c command) []byte {
	buf := make([]byte, 0, 1+4+len(c.Key)+4+len(c.Value)+8+4+len(c.LeaseID)+8+8)
	...
	buf = binary.BigEndian.AppendUint64(buf, uint64(c.ExpiresAtMs))
	return buf
}
...
func decodeCommand(data []byte) (command, error) {
	...
	if off+8 <= len(data) {
		c.ExpiresAtMs = int64(binary.BigEndian.Uint64(data[off : off+8]))
	}
	return c, nil
}
