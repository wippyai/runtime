...
type state struct {
	...
	leases  map[cluster.LeaseID]*leaseState
	version cluster.Version // global monotonic revision counter
}
...
func newState() *state {
	return &state{
		...
		leases:  make(map[cluster.LeaseID]*leaseState),
	}
}

// nextVersion increments and returns the global version.
func (s *state) nextVersion() cluster.Version {
	s.version++
	return s.version
}
...
// and the new version.
func (s *state) set(key string, value []byte, leaseID cluster.LeaseID) (*entry, cluster.Version) {
	ver := s.nextVersion()
	prev := s.entries[key]
...
func (s *state) setIfAbsent(key string, value []byte, leaseID cluster.LeaseID) (cluster.Version, bool) {
...
func (s *state) cas(key string, expect cluster.Version, value []byte) (cluster.Version, bool) {
	existing := s.entries[key]

	var actualVersion cluster.Version
	if existing != nil {
...
// scan iterates entries matching a prefix. Returns false if iteration was
// stopped by the callback.
func (s *state) scan(prefix string, fn func(cluster.Entry) bool) {
...
func (s *state) addLease(id cluster.LeaseID, ttlMs int64) {
...
func (s *state) removeLease(id cluster.LeaseID) []string {
	ls, ok := s.leases[id]
	if !ok {
		return nil
	}
...
func (s *state) snapshot() *stateSnapshot {
	snap := &stateSnapshot{
		entries: make(map[string]*cluster.Entry, len(s.entries)),
	}
	for k, e := range s.entries {
		snap.entries[k] = &cluster.Entry{
			Key:     e.key,
...
type stateSnapshot struct {
	entries map[string]*cluster.Entry
}

func (s *stateSnapshot) get(key string) *cluster.Entry {
	if s == nil {
		return nil
	}
	return s.entries[key]
...
func (s *stateSnapshot) scan(prefix string, fn func(cluster.Entry) bool) {
	if s == nil {
		return
	}
	for k, e := range s.entries {
