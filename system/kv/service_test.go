...
func TestService_GetNotFound(t *testing.T) {
	...
	assert.ErrorIs(t, err, cluster.ErrKeyNotFound)
}
...
func TestService_SetIfAbsent(t *testing.T) {
	...
	require.NoError(t, err)
	assert.False(t, ok2)
	assert.Equal(t, cluster.Version(1), ver2)
...
func TestService_Scan(t *testing.T) {
	...
	var users []string
	err := svc.Scan("users/", func(e cluster.Entry) bool {
...
func TestService_ScanEmpty(t *testing.T) {
	...
	var count int
	err := svc.Scan("anything/", func(e cluster.Entry) bool {
		count++
		return true
	})
	require.NoError(t, err)
...
func TestService_ScanAll(t *testing.T) {
	...
	var keys []string
	err := svc.Scan("", func(e cluster.Entry) bool {
...
func TestService_ScanEarlyStop(t *testing.T) {
	...
	var count int
	_ = svc.Scan("key", func(e cluster.Entry) bool {
		count++
		return count < 3
	})
...
func TestService_SetWithLease_InvalidLease(t *testing.T) {
	...
	assert.ErrorIs(t, err, cluster.ErrLeaseNotFound)
}
...
func TestService_Watch(t *testing.T) {
	...
	select {
	case evt := <-w.Events():
		assert.Equal(t, cluster.WatchPut, evt.Type)
...
func TestService_WatchDelete(t *testing.T) {
	...
	select {
	case evt := <-w.Events():
		assert.Equal(t, cluster.WatchDelete, evt.Type)
	...
	for i := 0; i < 20; i++ {
		...
		go func() {
			...
			for j := 0; j < 1000; j++ {
				...
				_ = svc.Scan("key", func(e cluster.Entry) bool { return true })
			}
		}()
	}
	wg.Wait()
...
func TestService_VersionMonotonicallyIncreasing(t *testing.T) {
	...
	var lastVersion cluster.Version
	for i := 0; i < 50; i++ {
