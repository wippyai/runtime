...
func testStrongChangedAdmission(t *testing.T, reject bool) {
	...
	hdr := pendingHeader{PID: p.String(), Name: "claim", AttemptID: testStrongAttemptID, RequiredNodes: []pid.NodeID{"node-1"}}
	...
	r.engine = &beforePromotionEngine{Engine: base, before: func() {
		...
		if reject {
			if _, err := base.Set(rejectKey("claim", hdr.AttemptID, "node-1"), []byte(strongRejectConflict)); err != nil {
				t.Fatal(err)
			}
		} else {
			if err := base.Delete(ackKey("claim", hdr.AttemptID, "node-1")); err != nil {
				t.Fatal(err)
			}
		}
	}}
