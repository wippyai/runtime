...
func TestRegisterStrong_SingleNode(t *testing.T) {
	...
	out, err := svc.RegisterScope(ctx, "system.root", p, global.Strong)
...
func TestRegisterStrong_TimeoutMissingAck(t *testing.T) {
	orig := global.StrongDeadline
	global.StrongDeadline = 400 * time.Millisecond
	defer func() { global.StrongDeadline = orig }()
	...
	_, err := svc.RegisterScope(ctx, "system.timeout", p, global.Strong)
	require.Error(t, err)
...
func TestRegisterStrong_RebroadcastIdempotent(t *testing.T) {
	...
	out, err := svc.RegisterScope(ctx, "system.idem", p, global.Strong)
	require.NoError(t, err)
	require.Equal(t, global.RegisterStateActive, out.State)
