...
func TestScopeTable_Consistent(t *testing.T) {
	...
	out, err := svc.RegisterScope(context.Background(), "svc", p, global.Consistent)
	require.NoError(t, err)
	assert.Equal(t, p, out.PID)
	assert.Equal(t, global.RegisterStateActive, out.State)
	...
	removed, err := svc.UnregisterScope(context.Background(), "svc", global.Consistent)
	require.NoError(t, err)
...
func TestConsistentScope_ConcurrentCrossNode(t *testing.T) {
	...
	go func() {
		start.Wait()
		out, err := svcA.RegisterScope(context.Background(), name, pA, global.Consistent)
		results <- result{who: "A", out: out, err: err}
	}()

	go func() {
		start.Wait()
		out, err := svcB.RegisterScope(context.Background(), name, pB, global.Consistent)
	...
	for _, r := range []result{r1, r2} {
		switch {
		case r.err == nil && r.out.State == global.RegisterStateActive:
			wins++
			winner = r
		case errors.Is(r.err, global.ErrNameAlreadyRegistered):
			conflicts++
			loser = r
		default:
