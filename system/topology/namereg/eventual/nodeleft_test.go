...
func entryDeleted(t *testing.T, st *eventual.State, name string) (deleted, present bool) {
	t.Helper()
	for i := 0; i < eventual.ShardCount; i++ {
...
func remoteDeltaFrame(t *testing.T, name string, p pid.PID, origin string, counter uint64) []byte {
	...
	body, err := eventual.EncodeDelta(nil, e, origin)
...
func TestNodeLeft_ReapPropagatesWithDepartedOrigin(t *testing.T) {
	...
	a := eventual.NewService(eventual.Config{LocalNodeID: "node-A", Bus: busA})
	c := eventual.NewService(eventual.Config{LocalNodeID: "node-C"})
