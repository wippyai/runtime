...
// revokeRecord captures a delivered revocation cancel: the target process and
// the reason carried by the cancel event.
type revokeRecord struct {
	Target pid.PID
	Reason string
}

func (r *recordingReceiver) revokes() []revokeRecord {
	r.mu.Lock()
	defer r.mu.Unlock()
	...
	for _, pkg := range r.pkgs {
		for _, m := range pkg.Messages {
			...
			for _, pl := range m.Payloads {
				if ev, ok := pl.Data().(*topology.CancelEvent); ok {
					out = append(out, revokeRecord{Target: pkg.Target, Reason: ev.Reason})
				}
			}
		}
	}
...
func TestRevoke_OnIncomingConflict(t *testing.T) {
	...
	assert.Equal(t, localPID, revokes[0].Target)
	assert.Contains(t, revokes[0].Reason, "svc.leader")
}
...
func TestRevoke_DedupeOnReapply(t *testing.T) {
	rec := &recordingReceiver{}
	svc := eventual.NewService(eventual.Config{LocalNodeID: "node-A", Revoker: rec})
	require.NoError(t, svc.Start(context.Background()))
	defer svc.Stop()
...
func TestRevoke_OnImmediateRegisterLoss(t *testing.T) {
	...
	require.Error(t, err)
	assert.Equal(t, eventual.ErrNameAlreadyRegistered, err)
