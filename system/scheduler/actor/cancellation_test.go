...
// TestActorConcurrentCancellationStress tests that canceling requests during execution
...
func TestActorConcurrentCancellationStress(t *testing.T) {
	...
	for i := 0; i < 1000; i++ {
		...
		go func(id int) {
			...
			// Some requests get canceled quickly, others run to completion
			...
			if err != nil {
				// Submit failed (context canceled before submit)
				return
			}
		}(i)
	}
