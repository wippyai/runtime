...
func TestLifecycleOnStartError(t *testing.T) {
	lc := &testLifecycle{
		onStart: func(_ context.Context, _ pidapi.PID, _ process.Process) {
			// This will be ignored, we need a different approach
		},
	}

	// Create a lifecycle that returns error
	type rejectingLifecycle struct{}
	_ = lc // unused but shows pattern
