...
func TestRegistry_ConcurrentRegister(t *testing.T) {
	...
	for i := range numHandlers {
		...
		}(dispatcher.CommandID(i))
	}

	wg.Wait()

	for i := range numHandlers {
		require.NotNil(t, r.Get(dispatcher.CommandID(i)), "handler %d should exist", i)
	}
}
...
func TestRegistry_ConcurrentGet(t *testing.T) {
	...
	const numHandlers = 100
	for i := range numHandlers {
		r.Register(dispatcher.CommandID(i), h)
	}
	r.Freeze()
	...
	for g := 0; g < numGoroutines; g++ {
		go func() {
			...
			for i := 0; i < numIterations; i++ {
				id := dispatcher.CommandID(i % numHandlers)
				got := r.Get(id)
				if got != h {
