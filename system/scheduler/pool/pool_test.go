...
func TestElasticBasic(t *testing.T) {
	pool, err := NewElastic(newMockFactory(0), &mockDispatcher{}, ElasticConfig{MinWorkers: 1, MaxWorkers: 4})
	if err != nil {
...
func TestElasticScalesUp(t *testing.T) {
	factory, count := newCountingFactory()
	pool, err := NewElastic(factory, &mockDispatcher{}, ElasticConfig{
		MinWorkers:  1,
		MaxWorkers:  4,
	...
	// Initial should have minWorkers
	time.Sleep(10 * time.Millisecond)
	if count.Load() < 1 {
		t.Fatalf("expected at least 1 process at start")
	}
}

func TestElasticConcurrent(t *testing.T) {
	pool, err := NewElastic(newMockFactory(0), &mockDispatcher{}, ElasticConfig{
		MinWorkers: 1,
		MaxWorkers: 8,
	})
	if err != nil {
...
func TestWorkStealingBasic(t *testing.T) {
	pool, err := NewWorkStealing(newMockFactory(0), &mockDispatcher{}, WorkStealingConfig{Workers: 2})
	if err != nil {
...
func TestWorkStealingConcurrent(t *testing.T) {
	pool, err := NewWorkStealing(newMockFactory(0), &mockDispatcher{}, WorkStealingConfig{Workers: 4})
	if err != nil {
		t.Fatalf("NewWorkStealing: %v", err)
	}
	defer pool.Stop()
...
func TestWorkStealingCancelation(t *testing.T) {
	pool, err := NewWorkStealing(newMockFactory(100*time.Millisecond), &mockDispatcher{}, WorkStealingConfig{Workers: 1})
	if err != nil {
		t.Fatalf("NewWorkStealing: %v", err)
	}
	defer pool.Stop()
	...
	// Fill up the worker
	go pool.Call(context.Background(), "blocking", nil)
	time.Sleep(10 * time.Millisecond)
...
func BenchmarkElasticCall(b *testing.B) {
	pool, _ := NewElastic(newMockFactory(0), &mockDispatcher{}, ElasticConfig{MinWorkers: 1, MaxWorkers: 8})
	defer pool.Stop()
	pool.Start()
...
func BenchmarkWorkStealingCall(b *testing.B) {
	pool, _ := NewWorkStealing(newMockFactory(0), &mockDispatcher{}, WorkStealingConfig{Workers: 4})
	defer pool.Stop()
	pool.Start()
