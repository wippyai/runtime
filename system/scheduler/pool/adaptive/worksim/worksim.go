...
type Workload struct {
	mu         sync.Mutex
	sem        chan struct{} // nil = no bottleneck
	bottleneck int

	// Latency configuration (atomic for lock-free reads)
	latencyNs atomic.Int64  // base latency in nanoseconds
	jitterPct atomic.Uint32 // jitter as percentage (0-100)

	// Metrics
	completed atomic.Int64
	active    atomic.Int32
	waiting   atomic.Int32
	maxActive atomic.Int32 // high water mark
}
...
// Returns context error if cancelled while waiting.
