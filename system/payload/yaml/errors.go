...
// Error implements apierror.Error for YAML payload errors
type Error struct {
	kind      apierror.Kind
	message   string
...
// NewUnmarshalError creates an error when YAML unmarshaling fails
func NewUnmarshalError(dataType string, err error) *Error {
	return &Error{
		kind:      apierror.KindInvalid,
		message:   "failed to unmarshal YAML " + dataType + ": " + err.Error(),
		retryable: apierror.False,
...
// NewMarshalError creates an error when YAML marshaling fails
func NewMarshalError(err error) *Error {
	return &Error{
		kind:      apierror.KindInternal,
		message:   "failed to marshal to YAML: " + err.Error(),
		retryable: apierror.False,
