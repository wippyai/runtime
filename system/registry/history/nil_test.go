...
func TestNilHistory_SaveWithoutHead(t *testing.T) {
	hist := NewNil()
...
func TestNilHistory_Get(t *testing.T) {
	hist := NewNil()

	v0 := version.New(registry.RootVersion)
	changes := registry.ChangeSet{
		{Kind: registry.Create, Entry: registry.Entry{ID: registry.ID{Name: "/test"}}},
	}
	...
	_, err := hist.Get(v0)
	if err == nil {
		t.Error("Get should return error for NilHistory, got nil")
	}
	if err.Error() != "version history not available" {
		t.Errorf("Expected 'version history not available' error, got: %v", err)
	}
}

func TestNilHistory_Versions(t *testing.T) {
	hist := NewNil()
	...
	_, err := hist.Versions()
	if err == nil {
		t.Error("Versions should return error for NilHistory, got nil")
	}
	if err.Error() != "version history not available" {
		t.Errorf("Expected 'version history not available' error, got: %v", err)
	}
...
func TestNilHistory_SetHead(t *testing.T) {
	hist := NewNil()
	...
	err := hist.SetHead(v0)
	if err == nil {
		t.Error("SetHead should return error for NilHistory, got nil")
	}
	if err.Error() != "rewinding not supported with nil history" {
		t.Errorf("Expected 'rewinding not supported' error, got: %v", err)
	}
...
func TestNilHistory_ConcurrentHeadRead(t *testing.T) {
	hist := NewNil()
