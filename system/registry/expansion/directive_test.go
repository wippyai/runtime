...
func TestDependencyDirective_Expand(t *testing.T) {
	called := false
	d := NewDependencyDirective(func(ctx context.Context, op registry.Operation, snapshot registry.ProvenancedState) (registry.DirectiveResult, error) {
		called = true
		return registry.DirectiveResult{Applied: true}, nil
	})

	result, err := d.Expand(context.Background(), registry.Operation{}, registry.ProvenancedState{})
...
func TestDependencyDirective_Expand_CancelledContext(t *testing.T) {
	...
	_, err := d.Expand(ctx, registry.Operation{}, registry.ProvenancedState{})
	assert.ErrorIs(t, err, context.Canceled)
}

func TestDependencyDirective_ReconcileResolutionCompatibility(t *testing.T) {
	called := false
	d := NewDependencyDirective(nil, func(_ context.Context, snapshot registry.ProvenancedState, _ *registry.DependencyResolution) (registry.DirectiveResult, error) {
		called = true
		require.Len(t, snapshot.Entries, 1)
		return registry.DirectiveResult{Applied: true}, nil
	})

	result, err := d.ReconcileResolution(context.Background(), registry.ProvenancedState{Entries: registry.State{{Kind: "service"}}}, nil)
	require.NoError(t, err)
...
func TestDependencyDirective_ReconcileResolutionTransitionKeepsSourceAndTarget(t *testing.T) {
	current := registry.ProvenancedState{Entries: registry.State{{ID: registry.NewID("app", "before"), Kind: "service"}}}
	target := registry.ProvenancedState{Entries: registry.State{{ID: registry.NewID("app", "after"), Kind: "service"}}}
	d := NewDependencyDirective(nil).WithResolutionTransition(
		func(_ context.Context, gotCurrent registry.ProvenancedState, gotTarget registry.ProvenancedState, _ *registry.DependencyResolution) (registry.DirectiveResult, error) {
