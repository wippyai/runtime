...
func TestDependencyExtraction_EnvPattern(t *testing.T) {
	sdk := NewDepTestSDK(t)
	...
	t.Run("Basic *_env Pattern", func(t *testing.T) {
		...
		sdk.ExpectDeps(`{
			...
			"kind": "service",
			"data": {
				"app_env": "app.env:file"
			}
		}`).ToEqual("app.env:file")

		// Multiple *_env fields
		sdk.ExpectDeps(`{
			"id": "test", 
			"kind": "service",
			"data": {
				"app_env": "app.env:file",
				"db_env": "db.env:os",
				"cache_env": "cache.env:file"
			}
		}`).ToEqual("app.env:file", "db.env:os", "cache.env:file")

		// *_env with array values
		sdk.ExpectDeps(`{
			"id": "test",
			"kind": "service", 
			"data": {
				"config_env": ["app.env:file", "app.env:os"]
			}
		}`).ToEqual("app.env:file", "app.env:os")
	})
	...
	t.Run("Non-matching Fields", func(t *testing.T) {
		...
		// Only fields ending with _env should match
		sdk.ExpectDeps(`{
			"id": "test",
			"kind": "service",
			"data": {
	...
	t.Run("*_env with Other Dependencies", func(t *testing.T) {
		...
		}`).ToEqual("gateway", "app.env:file")

		// *_env with data.store (existing path)
		sdk.ExpectDeps(`{
			"id": "test",
			"kind": "service",
			"data": {
				"store": "cache",
				"config_env": "config.env:file"
			}
		}`).ToEqual("cache", "config.env:file")

		// *_env with data.middleware (existing path)
		sdk.ExpectDeps(`{
			"id": "test",
			"kind": "service",
			"data": {
				"middleware": ["auth", "logging"],
				"secrets_env": "secrets.env:os"
			}
		}`).ToEqual("auth", "logging", "secrets.env:os")
	})
	...
	t.Run("*_env with Meta Dependencies", func(t *testing.T) {
		...
		}`).ToEqual("gateway", "db.env:file")

		// *_env with meta.depends_on
		sdk.ExpectDeps(`{
			"id": "test",
			"kind": "service",
			"meta": {
				"depends_on": ["manual-dep"]
			},
			"data": {
				"app_env": "app.env:file"
			}
		}`).ToEqual("manual-dep", "app.env:file")
	})
	...
	t.Run("Edge Cases", func(t *testing.T) {
		...
		}`).ToEqual("cache.env:file")

		// *_env with mixed empty and valid values in arrays
		sdk.ExpectDeps(`{
			"id": "test",
			"kind": "service",
			"data": {
	...
	t.Run("Real World Scenarios", func(t *testing.T) {
		...
		sdk.ExpectDeps(`{
			...
			"data": {
				"database_env": "app.env:file",
				"credentials_env": "app.env:os"
			}
		}`).ToEqual("app.env:file", "app.env:os")

		// Service with multiple env types
		sdk.ExpectDeps(`{
			"id": "app:api",
			"kind": "http.service",
			"data": {
				"app_env": "app.env:file",
				"secrets_env": "app.env:os",
				"config_env": "config.env:file"
			}
		}`).ToEqual("app.env:file", "app.env:os", "config.env:file")
		...
		sdk.ExpectDeps(`{
			...
			"data": {
				"store": "session",
				"worker_env": "worker.env:file",
				"db_env": "db.env:os"
			}
		}`).ToEqual("database", "session", "worker.env:file", "db.env:os")
	})
}
