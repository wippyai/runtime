...
// TestRootPending_HappyPath drives a pending → ack → active transition
...
// the fence token.
func TestRootPending_HappyPath(t *testing.T) {
	fsm := NewFSM()
	...
	gotPID, gotIdx, found := fsm.State().LookupWithFence("root.svc")
	require.True(t, found)
	assert.Equal(t, p, gotPID)
	assert.Equal(t, uint64(13), gotIdx, "fence token equals the activation index (last ack)")
}

// TestRootPending_ExpiryMissingOneAck verifies that the FSM releases the
// reservation on expiry and reports exactly the missing node.
func TestRootPending_ExpiryMissingOneAck(t *testing.T) {
	fsm := NewFSM()
	p := makePID("node-1", "worker", "p1")
	required := []pid.NodeID{"node-1", "node-2", "node-3"}
	...
	_, _, found := fsm.State().LookupWithFence("root.svc")
...
// TestRootPending_ConcurrentConflict proves that two different PIDs
// cannot both reserve the same root name: the second loses.
func TestRootPending_ConcurrentConflict(t *testing.T) {
	fsm := NewFSM()
...
// TestRootPending_DeterministicSnapshot proves that a replay of the same
// pending command yields the same RequiredNodes view on every replica.
func TestRootPending_DeterministicSnapshot(t *testing.T) {
...
// TestRootPending_IdempotentAcks verifies that re-applying the same ack
// does not double-count nor re-trigger activation.
func TestRootPending_IdempotentAcks(t *testing.T) {
	fsm := NewFSM()
	required := []pid.NodeID{"node-1", "node-2"}
	applyAt(t, fsm, &Command{
		Type:          CmdRegisterPending,
	...
	_, idx, found := fsm.State().LookupWithFence("root.idem")
	require.True(t, found, "activated entry should be looked up")
	assert.Equal(t, uint64(53), idx)
}

// TestRootPending_SnapshotRoundTrip verifies that pending reservations
// survive a Snapshot/Restore cycle.
func TestRootPending_SnapshotRoundTrip(t *testing.T) {
	fsm := NewFSM()
