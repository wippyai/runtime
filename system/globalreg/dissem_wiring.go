...
const (
	...
	topicLookupRequest relay.Topic = "globalreg.lookup.req"
	...
	topicLookupResponse relay.Topic = "globalreg.lookup.resp"
	...
	topicDigestExchange relay.Topic = "globalreg.dissem.digest"
	...
	topicDigestDelta relay.Topic = "globalreg.dissem.delta"
)
...
func (s *Service) forwardLookup(ctx context.Context, name string) (globalreg.LookupResult, bool) {
	targets, err := s.resolveForwardTarget()
	if err != nil || len(targets) == 0 {
		return globalreg.LookupResult{}, false
	}
	...
	for _, target := range targets {
		...
		case resp := <-respCh:
			...
			return globalreg.LookupResult{PID: resp.PID, Found: true}, true
