...
func (t *ToGolang) Transcode(p payload.Payload) (payload.Payload, error) {
	if p.Format() != payload.Json {
		return nil, fmt.Errorf("ToGolang can only transcode from JSON format, got %s", p.Format())
	}
...
func (t *ToGolang) Unmarshal(p payload.Payload, v interface{}) error {
	if p.Format() != payload.Json {
		return fmt.Errorf("ToGolang can only unmarshal from JSON format, got %s", p.Format())
	}
